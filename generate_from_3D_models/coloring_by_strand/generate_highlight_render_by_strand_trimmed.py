...
#
# This is a copy of generate_highlight_render_by_strand.py that additionally
# accepts --ignore_npz: an npz with an (nr_strands,) bool array "ignore",
# aligned 1:1 with --input_npz / --template_npz (same nr_strands, same
# order). Strands with ignore[i] == True are dropped ENTIRELY before
# rendering (no curve is even created for them) - unlike --template_npz's
# `highlighted`, which only controls color, `ignore` removes the strand's
# geometry outright. Dropping happens before --strands_subsample, so the
# subsample fraction is drawn from the already-trimmed strand set.
...
def blender_main():
    ...
    parser.add_argument('--strands_subsample', type=float, default=0.3,
                         help='Fraction of strands to keep, for a faster render - the SAME random draw is applied '
                              ...
                              'stay index-aligned with the kept strands. Applied AFTER --ignore_npz.')
    ...
    parser.add_argument('--ignore_npz', default=None,
                         help='optional npz with an (nr_strands,) bool array "ignore", index-aligned with '
                              '--input_npz/--template_npz (same nr_strands, same order). Strands with ignore[i] '
                              'True are dropped ENTIRELY before rendering (no curve created for them at all) - '
                              'unlike --template_npz\'s "highlighted", which only changes color. Applied before '
                              '--strands_subsample.')
    ...
    if args.ignore_npz:
        ignore_data = np.load(args.ignore_npz)
        ignore_mask = np.asarray(ignore_data["ignore"]).astype(bool)
        if ignore_mask.shape[0] != nr_strands:
            raise RuntimeError(
                f"--ignore_npz has {ignore_mask.shape[0]} entries but --input_npz has {nr_strands} strands - "
                f"the ignore mask must be index-aligned 1:1 with --input_npz/--template_npz.")
        keep = ~ignore_mask
        points = points[keep].copy()
        root_uv = root_uv[keep].copy()
        template_highlighted = template_highlighted[keep].copy()
        template_highlight_color = template_highlight_color[keep].copy()
        nr_strands = points.shape[0]
        print(f"--ignore_npz: dropped {int(ignore_mask.sum())}/{ignore_mask.shape[0]} strands before rendering "
              f"({nr_strands} remain)")
